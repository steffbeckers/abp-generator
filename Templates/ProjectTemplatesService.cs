...
public class ProjectTemplatesService
{
    ...
    public Task GenerateAsync(ProjectTemplateGenerateInputDto input)
    {
        ...
        if (template == null || string.IsNullOrEmpty(template.FullPath)) return Task.CompletedTask;
        ...
        return Parallel.ForEachAsync(templateSourceFilePaths, async (sourceFilePath, cancellationToken) =>
        {
